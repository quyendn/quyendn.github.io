...
$(document).ready(function() {
    ...
    $('#frmRegDownload').bootstrapValidator({
        ...
        onSuccess: function(e) {
            ...
            var description = $('#txtComp').val();
            var emailto = "quyendn84@gmail.com";
            var isSendMail = 1;
            var url_source = "lp_daiphat";
            var source = getUrlParameter('utm_source');
            if (isEmpty(source))
                url_source = "lp_daiphat";
            else
                url_source = source;
            var check = checkPhoneNumber();
            if (!check)
                return;
            var dataJSON = { "name": name, "phone": phone, "email": email, "description": description, "product": product, "source": url_source, "emailto": emailto };
            showLoadingContactImage('content-register', 'frmContentReg');
            $.ajax({
                url: "https://alpha.f5academy.net/api/DaiPhatservice",
                type: "Post",
                async: false,
                data: dataJSON,
                ...
                complete: function(jqXHR, textStatus) {
                    ...
                    hideLoadingContactImage('content-register', 'frmContentReg');
                    window.location.href = "https://quyendn.github.io/datphat/dang-ky-thanh-cong.html";
                    $("#register-popup").hide();
                }
            });
        }
