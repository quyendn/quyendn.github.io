...
$(document).ready(function() {
    ...
    $('#frmRegDownload').bootstrapValidator({
        ...
        onSuccess: function(e) {
            ...
            var model = $("#txtEmail").val();
            var phone = $('#txtPhone').val();
            var emailto = "quyendn84@gmail.com";
            var check = checkPhoneNumber();
            if (!check)
                return;
            var url_source = "lp_agribank";
            var source = getUrlParameter('utm_source');
            if (isEmpty(source))
                url_source = "lp_agribank";
            else
                url_source = source;
            var dataJSON = { "name": name, "address": address,"phone": phone, "model": model,"url_source":url_source, "emailto": emailto };
            showLoadingContactImage('content-register', 'frmContentReg');
            $.ajax({
                url: "https://alpha.f5academy.net/api/Agribankservice",
                type: "Post",
                async: false,
                data: dataJSON,
                ...
                complete: function(jqXHR, textStatus) {
                    ...
                    hideLoadingContactImage('content-register', 'frmContentReg');
                    window.location.href = "https://quyendn.github.io/agribank/dang-ky-thanh-cong.html";
                }
            });
        }
